...
- `title`：文章标题
- `date`：发布日期，可省略时由 `_posts/YYYY-MM-DD-title.md` 文件名提供日期
- `permalink`：页面链接，可改可不改
- `tags`：标签，可选

博客页会自动从 GitHub 提交记录解析时间：

- 创建时间：该文章 Markdown 文件在远程仓库里的第一次提交时间
- 更新时间：该文章 Markdown 文件在远程仓库里的最近一次提交时间

这两个时间不需要在 Markdown front matter 中手动填写；正常修改文章、提交并推送后，线上页面会自动查询并显示。
